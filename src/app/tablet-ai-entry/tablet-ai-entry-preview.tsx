...
function QuickButton({ children, top }: { children: React.ReactNode; top: number }) {
  return (
    <button
      className="absolute left-[88px] h-[60px] rounded-[8px] border border-[#dcdcdc] bg-white px-[14px] text-left text-[24px] leading-none text-[#2f2f2f]"
      style={{ top }}
      type="button"
    >
      {children}
...
function AiPanel() {
  return (
    <aside className="absolute left-[966px] top-0 h-[1200px] w-[954px] rounded-l-[12px] bg-white shadow-[-12px_0_24px_rgba(0,0,0,0.13)]">
      ...
      <QuickButton top={357}>帮我识别作业资料</QuickButton>
